  ...
  export const generateUsers = (count: number = 100): User[] => {
    return Array.from({ length: count }, (_, index) => {
      const employmentStatus = faker.helpers.arrayElement(["Employed", "Unemployed", "Self-Employed", "Retired", "Student"] as const);
      const noIncome = employmentStatus === "Retired" || employmentStatus === "Unemployed";
      const incomeBase = noIncome ? 0 : faker.number.float({ min: 1000, max: 5000000 });
      ...
      organization: faker.helpers.arrayElement(["lendsqr", "irorun", "lendstar"]),
      ...
      balance: faker.number.float({ min: 1000, max: 500000, fractionDigits: 2 }),
      ...
      typeOfResidence: faker.helpers.arrayElement(["Parent's Apartment", "Own Apartment", "Rent", "Business"]),
      levelOfEducation: faker.helpers.arrayElement(["Bachelor's", "Master's", "PhD", "High School", "Other"]),
      employmentStatus,
      ...
      guarantorRelationship: faker.helpers.arrayElement(["Parent", "Sibling", "Friend", "Other"]),
      ...
      secondGuarantorRelationship: faker.helpers.arrayElement(["Parent", "Sibling", "Friend", "Other"]),
      twitter: "@" + faker.internet.username().toLowerCase().replace(/\s/g, ""),
      facebook: faker.internet.username().replace(/_/g, " "),
      instagram: "@" + faker.internet.username().toLowerCase().replace(/\s/g, "_"),
      };
    });
  };
